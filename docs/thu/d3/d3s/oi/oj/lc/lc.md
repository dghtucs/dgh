...
5
!
5! 的过程当中，需要记录拆分的过程当中的每一个子问题，并且在求解每一个子问题以后，逐层向上汇报结果。后拆分的子问题先得到了解决，整个过程恰好符合「后进先出」的规律 ，因此需要借助的数据结构是「栈」。
...
大部分的树的问题，都是在遍历的过程中解决的。具体使用哪种方式遍历，需要结合具体的问题进行分析。

树的遍历是树上最基础的算法，本身非常简单，单独解决树的遍历问题，并不涉及到技巧。但是树的遍历的应用场景太多，很多经典问题背后其实就是树的遍历。因此这一章我们将「力扣」上的树的遍历问题根据应用场景进行梳理，从而掌握在实际问题中灵活应用树的遍历的技巧。

涉及到的经典问题有建树问题、结构判断问题、数据压缩问题、编码解码问题、序列化反序列化问题、自底向上整合子树结果、层序遍历的变种。
...
关于树的问题，还有两类问题非常能体现算法技巧，分别是树形 DP 和最近公共祖先
