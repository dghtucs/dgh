...
struct quadratic_probe : public collision_strategy {
    int step;
    void init() override { step = 0; }

    int operator()(hash_entry*, int table_size, int last) override {
        step++;
        int offset = step * step;
        if (step & 1)
            return (last + offset) % table_size;
        else
            return (last - offset + table_size) % table_size;
    }
};
...
struct overflow_area : public collision_strategy {
    ...
    overflow_area(int size) {
        overflow_start = size * 4 / 5;
    }
    ...
    int operator()(hash_entry*, int table_size, int) override {
        if (cur >= table_size) return -1;
        return cur++;
    }
};
